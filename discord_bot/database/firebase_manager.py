    ...
    def _initialize_firebase(self):
        ...
        try:
            ...
            if not firebase_database_url:
                logger.warning(
                    "FIREBASE_DATABASE_URL not set, falling back to local storage")
            ...
            if all([firebase_creds["project_id"], firebase_creds["private_key"], firebase_creds["client_email"]]):
                ...
                parent_cred_path = os.path.join(os.path.dirname(
                ...
                if os.path.isfile(local_cred_path):
                    ...
                    logger.info(
                        "Using local credentials.json from discord_bot directory")
                ...
                if cred_path:
                    ...
                    logger.warning(
                        "Firebase credentials not found, using local storage")
                    self.use_firebase = False
                    self._initialize_local()
    ...
    def load_tasks(self, username: str) -> List[Task]:
        ...
        if migrated_missing_uuid and tasks:
            try:
                ...
                logger.info(
                    f"Backfilled UUIDs for existing tasks of user {username}")
            except Exception as e:
                logger.warning(
                    f"Failed UUID backfill for user {username}: {e}")
        return tasks
    ...
    def reorder_tasks(self, username: str, task_ids: List[str]):
        ...
        for tid in task_ids:
            if task_map[tid].colour != group_colour:
                raise ValueError(
                    "Cannot reorder tasks across different priority groups")
